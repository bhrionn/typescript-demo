/**
 * Unit tests for Get File Metadata handler
 * Requirements: 2.3
 */
...
import { handler } from '../get-file-metadata';
...
describe('Get File Metadata Handler', () => {
  ...
  const createMockEvent = (overrides: Partial<AuthenticatedEvent> = {}): AuthenticatedEvent => ({
    ...
    path: '/api/files/123e4567-e89b-12d3-a456-426614174000',
    pathParameters: {
  ...
  describe('Successful metadata retrieval', () => {
    it('should return file metadata for valid file ID', async () => {
      ...
      expect(body.data.fileSize).toBe(5120);
      expect(body.data.mimeType).toBe('application/pdf');
      expect(body.data.uploadedAt).toBe('2024-01-15T10:30:00.000Z');
      expect(body.data.metadata).toEqual({ category: 'work', tags: ['important'] });
      expect(mockDb.disconnect).toHaveBeenCalled();
    });

    it('should handle file with null metadata', async () => {
      const mockFile = {
        id: '123e4567-e89b-12d3-a456-426614174000',
        userId: 'test-user-id',
        fileName: 'image.jpg',
  ...
  describe('Error handling', () => {
    it('should handle database errors', async () => {
      ...
      expect(body.message).toContain('Failed to retrieve file metadata');
      expect(mockDb.disconnect).toHaveBeenCalled();
    });
